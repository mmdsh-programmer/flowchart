...
const FileAttachment = (props: IProps) => {
  ...
  return (
    <div className="attach-file-wrapper">
      <p className="clasor-extension">
        {fileName}.{fileExtension}
      </p>
      <a
        className="clasor-download"
        target="_blank"
        rel="noreferrer"
        download
        href={`${podSpaceServer}files/${fileHash}?checkUserGroupAccess=true&Authorization=${token}&time=${+new Date()}`}
      >
      ...
      {open && (
        <PdfReader
          ...
          pdfSource={`${podSpaceServer}files/${fileHash}?checkUserGroupAccess=true&Authorization=${token}&time=${+new Date()}`}
        />
      )}
    </div>
  );
